...
world_reshape(int width, int height)
{
    ...
    glViewport(0, 0, width, height);
    exsetmatrixmode(GL_PROJECTION, TRUE);
	experspective(60.0, (GLfloat)width / height, 0.01, 256.0);
	exsetmatrixmode(GL_MODELVIEW, TRUE);
    extranslatef(0.0, 0.0, -6.0);
    exrotatef(-45.0, 0.0, 1.0, 0.0);
...
world_display(void)
{
    ...
    exlightmodelf(GL_LIGHT_MODEL_LOCAL_VIEWER, local_viewer.value);
    exlightmodelf(GL_LIGHT_MODEL_TWO_SIDE, two_side.value);
    exlightmodelfv(GL_LIGHT_MODEL_AMBIENT, lmKa);

    exmaterialfv(GL_FRONT, GL_AMBIENT, mKa);
    exmaterialfv(GL_FRONT, GL_DIFFUSE, mKd);
    exmaterialfv(GL_FRONT, GL_SPECULAR, mKs);
    exmaterialfv(GL_FRONT, GL_EMISSION, mKe);
    exmaterialf(GL_FRONT, GL_SHININESS, material_Se.value);

    exlightfv(GL_LIGHT0, GL_AMBIENT, lKa);
    exlightfv(GL_LIGHT0, GL_DIFFUSE, lKd);
    exlightfv(GL_LIGHT0, GL_SPECULAR, lKs);
    exlighti(GL_LIGHT0, GL_SPOT_EXPONENT, (int)spot_exponent.value);
    if (spot_cutoff.value > 90)
	exlighti(GL_LIGHT0, GL_SPOT_CUTOFF, 180);
    else
	exlighti(GL_LIGHT0, GL_SPOT_CUTOFF, (int)spot_cutoff.value);
    exlightf(GL_LIGHT0, GL_CONSTANT_ATTENUATION, Kc.value);
    exlightf(GL_LIGHT0, GL_LINEAR_ATTENUATION, Kl.value);
    exlightf(GL_LIGHT0, GL_QUADRATIC_ATTENUATION, Kq.value);
    ...
    expushmatrix();
    exmultmatrixd(inverse);
    extranslatef(l[0], l[1], l[2]);
    excolor3fv(lKd);
    exbeginmode(GL_LINE_STRIP);
    if (spot_cutoff.value > 90)
      exvertex3f(0, 0, 0);
    else
      exvertex3f(pos[0]+spot_direction[0].value,
		 pos[1]+spot_direction[1].value, 
		 pos[2]+spot_direction[2].value);

    if (pos[3] == 0)		/* 10.0 = 'infinite' light */
      exvertex3f(pos[0]*10.0,pos[1]*10.0,pos[2]*10.0);
    else 
      exvertex3f(pos[0], pos[1], pos[2]);
    exendmode();
    exlightfv(GL_LIGHT0, GL_SPOT_DIRECTION, dir);
    exlightfv(GL_LIGHT0, GL_POSITION, pos);
    expopmatrix();
	...
	if (pmodel)
	  ...
	  exglutsolidtorus(0.25, 0.75, 28, 28);
	exdisable(GL_LIGHTING);
    }
...
#define TESS 20
    ...
    for (i = 0; i < TESS; i++) {
	exbeginmode(GL_TRIANGLE_STRIP);
	for (j = 0; j <= TESS; j++) {
	    exvertex3f(-1+(float)i/TESS*2, -1.0, -1+(float)j/TESS*2);
	    exvertex3f(-1+(float)(i+1)/TESS*2, -1.0, -1+(float)j/TESS*2);
	}
	exendmode();
    }
#endif
    ...
    exbeginmode(GL_LINE_STRIP);
    exvertex3f(0.0, 0.0, 0.0);
    exvertex3f(0.0, 0.0, -1.0*length);
    exvertex3f(0.1, 0.0, -0.9*length);
    exvertex3f(-0.1, 0.0, -0.9*length);
    exvertex3f(0.0, 0.0, -1.0*length);
    exvertex3f(0.0, 0.1, -0.9*length);
    exvertex3f(0.0, -0.1, -0.9*length);
    exvertex3f(0.0, 0.0, -1.0*length);
    exendmode();
    excolor3ub(255, 255, 0);
    exrasterpos3f(0.0, 0.0, -1.1*length);
    exbitmapcharacter(GLUT_BITMAP_HELVETICA_12, 'e');
    excolor3ub(255, 0, 0);
    exscalef(0.4, 0.4, 0.4);
    drawaxes();
    expopmatrix();
    ...
    excolor3f(0.2, 0.2, 0.2);
    exbeginmode(GL_QUADS);
    exvertex3i(1, 1, 1);
    exvertex3i(-1, 1, 1);
    exvertex3i(-1, -1, 1);
    exvertex3i(1, -1, 1);
    ...
    excolor3ub(128, 196, 128);
    exbeginmode(GL_LINES);
    exvertex3i(1, 1, -1);
    exvertex3i(1, 1, 1);
    exvertex3i(-1, 1, -1);
    exvertex3i(-1, 1, 1);
...
screen_reshape(int width, int height)
{
    ...
    glViewport(0, 0, width, height);
    exsetmatrixmode(GL_PROJECTION, TRUE);
    experspective(60.0, (float)width/height, 0.5, 8.0);
    exgetparamd(GL_PROJECTION_MATRIX, projection);
    exsetmatrixmode(GL_MODELVIEW, TRUE);
...
screen_display(void)
{
    ...
    exgetparamd(GL_MODELVIEW_MATRIX, modelview);
    if (pmodel)
	drawmodel();
    else 
	exglutsolidtorus(0.25, 0.75, 28, 28);
...
lighting_display(void)
{
    ...
    drawstr(10, light_Ks[0].y+30,
	    "exlightfv(GL_LIGHT0, GL_POSITION, light_pos);");
    drawstr(10, light_Ks[1].y+50,
	    "exlightfv(GL_LIGHT0, GL_AMBIENT, light_Ka);");
    drawstr(10, light_Ks[2].y+70,
	    "exlightfv(GL_LIGHT0, GL_DIFFUSE, light_Kd);");
    drawstr(10, light_Ks[3].y+90,
	    "exlightfv(GL_LIGHT0, GL_SPECULAR, light_Ks);");
...
spotlight_display(void)
{
    ...
    drawstr(10, spot_cutoff.y+30,
	 "exlightfv(GL_LIGHT0, GL_SPOT_DIRECTION, spot_direction);");
    drawstr(10, spot_cutoff.y+50,
	 "exlighti(GL_LIGHT0, GL_SPOT_EXPONENT, spot_exponent);");
    drawstr(10, spot_cutoff.y+70,
	 "exlighti(GL_LIGHT0, GL_SPOT_CUTOFF, spot_cutoff);");
    ...
    setfont("helvetica", 12);
    drawstr(10, Kq.y+30, "exlightf(GL_LIGHT0, GL_CONSTANT_ATTENUATION, Kc);");
    drawstr(10, Kq.y+50, "exlightf(GL_LIGHT0, GL_LINEAR_ATTENUATION, Kl);");
    drawstr(10, Kq.y+70, "exlightf(GL_LIGHT0, GL_QUADRATIC_ATTENUATION, Kq);");
...
material_display(void)
{
    ...
    drawstr(10, material_Se.y+30,
	 "exmaterialfv(GL_FRONT, GL_AMBIENT, material_Ka);");
    drawstr(10, material_Se.y+50,
	 "exmaterialfv(GL_FRONT, GL_DIFFUSE, material_Kd);");
    drawstr(10, material_Se.y+70,
	 "exmaterialfv(GL_FRONT, GL_SPECULAR, material_Ks);");
    drawstr(10, material_Se.y+90,
	 "exmaterialfv(GL_FRONT, GL_EMISSION, material_Ke);");
    drawstr(10, material_Se.y+110,
	 "exmaterialfv(GL_FRONT, GL_SHININESS, material_Se);");
...
lmodel_display(void)
{
    ...
    drawstr(10, lmodel_Ka[3].y+30,
	 "exlightmodelfv(GL_LIGHT_MODEL_AMBIENT, lmodel_Ka);");

    setfont("helvetica", 18);
    drawstr(10, local_viewer.y, "exlightmodelf(GL_LIGHT_MODEL_LOCAL_VIEWER, ");
    drawstr(local_viewer.x+35, local_viewer.y, ");");

    drawstr(10, two_side.y, "exlightmodelf(GL_LIGHT_MODEL_TWO_SIDE, ");
