...
glmDraw(GLMmodel* model, GLuint mode)
{
  ...
  while (group) {
    if (mode & GLM_MATERIAL) {
      ...
      exmaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, material->ambient);
      exmaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, material->diffuse);
      exmaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, material->specular);
      exmaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, &material->shininess);
    }

    if (mode & GLM_COLOR) {
      excolor3fv(material->diffuse);
    }
    ...
    for (i = 0; i < group->numtriangles; i++) {
      ...
      if (mode & GLM_FLAT)
	exnormal3fv(&model->facetnorms[3 * triangle->findex]);
      
      if (mode & GLM_SMOOTH)
	exnormal3fv(&model->normals[3 * triangle->nindices[0]]);
      if (mode & GLM_TEXTURE)
	extexcoord2fv(&model->texcoords[2 * triangle->tindices[0]]);
      exvertex3fv(&model->vertices[3 * triangle->vindices[0]]);
      
      if (mode & GLM_SMOOTH)
	exnormal3fv(&model->normals[3 * triangle->nindices[1]]);
      if (mode & GLM_TEXTURE)
	extexcoord2fv(&model->texcoords[2 * triangle->tindices[1]]);
      exvertex3fv(&model->vertices[3 * triangle->vindices[1]]);
      
      if (mode & GLM_SMOOTH)
	exnormal3fv(&model->normals[3 * triangle->nindices[2]]);
      if (mode & GLM_TEXTURE)
	extexcoord2fv(&model->texcoords[2 * triangle->tindices[2]]);
      exvertex3fv(&model->vertices[3 * triangle->vindices[2]]);
