...
world_reshape(int width, int height)
{
    ...
    experspective(60.0, (GLfloat)width/height, 1.0, 256.0);
    exsetmatrixmode(GL_MODELVIEW, TRUE);
...
screen_reshape(int width, int height)
{
    ...
    if (mode == PERSPECTIVE)
	experspective(perspective[0].value, perspective[1].value, 
		       perspective[2].value, perspective[3].value);
    else if (mode == ORTHO)
	exortho(ortho[0].value, ortho[1].value, ortho[2].value,
		ortho[3].value, ortho[4].value, ortho[5].value);
    else if (mode == FRUSTUM)
	exfrustum(frustum[0].value, frustum[1].value, frustum[2].value,
...
command_display(void)
{
    ...
    if (mode == PERSPECTIVE) {
	drawstr(40, perspective[0].y, "experspective(");
    ...
    drawstr(78, lookat[0].y, "exlookat(");
...
main(int argc, char** argv)
{
    ...
    glutAddMenuEntry("[p]  experspective", 'p');
    glutAddMenuEntry("", 0);
