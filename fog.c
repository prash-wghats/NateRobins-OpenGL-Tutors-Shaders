...
world_reshape(int width, int height)
{
    ...
    exortho(0, width, height, 0, -1.0, 1.0);
    exsetmatrixmode(GL_MODELVIEW, TRUE);
...
screen_reshape(int width, int height)
{
    ...
    exenable(GL_LIGHT0);
    exenable(GL_FOG);
    exfogi(GL_FOG_MODE, mode);
    exfogf(GL_FOG_START, fstart.value);
    exfogf(GL_FOG_END, fend.value);
    exfogf(GL_FOG_DENSITY, density.value);

    cell_vector(fog_color, color, 4);
    exfogfv(GL_FOG_COLOR, fog_color);
}
